# Exercise and Checklist Library — Data Transfer, Memory, and the CPU-GPU Boundary

Read this during Phase 5 (exercises) and Phase 6 (checklists). Adapt
scenarios to the audience's domain; keep evaluation criteria intact — they
are what make the exercises assessable. In full-module mode, write an
expert-quality model solution for each included exercise.

## Table of Contents
- Part A: Practical Exercises
- Part B: Professional Gate Checklists

---

# Part A: Practical Exercises

## Exercise 1 — Engineer the CPU-GPU boundary through typed-array layouts (interleaved versus planar
**Scenario:** provide a realistic Graphics APIs scenario appropriate to the audience (real or closely modeled on documented practice).
**Objective:** engineer the CPU-GPU boundary through typed-array layouts (interleaved versus planar, quantization with precision budgets), upload mechanics (partial updates, PBO async staging, static-once discipline), and web-worker architectures with zero-copy transferables and OffscreenCanvas rendering.
**Constraints:** must use the framework taught in the corresponding unit; must state assumptions and trade-offs; one deliverable, length-bounded.
**Deliverable:** the professional artifact this capability produces (memo, table, plan, analysis, or model — as applicable).
**Evaluation criteria:** correct application of the framework; decisions justified with evidence; limitations honestly stated; artifact usable by a colleague without further explanation.

## Exercise 2 — Integrate Arrow columnar pipelines and binary transport with progressive loading and caching
**Scenario:** provide a realistic Graphics APIs scenario appropriate to the audience (real or closely modeled on documented practice).
**Objective:** integrate Arrow columnar pipelines and binary transport with progressive loading and caching.
**Constraints:** must use the framework taught in the corresponding unit; must state assumptions and trade-offs; one deliverable, length-bounded.
**Deliverable:** the professional artifact this capability produces (memo, table, plan, analysis, or model — as applicable).
**Evaluation criteria:** correct application of the framework; decisions justified with evidence; limitations honestly stated; artifact usable by a colleague without further explanation.

## Exercise 3 — Manage memory budgets across JS-heap and GPU resources with leak prevention and eviction strategies
**Scenario:** provide a realistic Graphics APIs scenario appropriate to the audience (real or closely modeled on documented practice).
**Objective:** manage memory budgets across JS-heap and GPU resources with leak prevention and eviction strategies, orchestrate frame-loop data flow (uniform-only view updates, ring-buffer streaming, backpressure policies), and profile bandwidth-bound versus compute-bound bottlenecks driving optimization targeting.
**Constraints:** must use the framework taught in the corresponding unit; must state assumptions and trade-offs; one deliverable, length-bounded.
**Deliverable:** the professional artifact this capability produces (memo, table, plan, analysis, or model — as applicable).
**Evaluation criteria:** correct application of the framework; decisions justified with evidence; limitations honestly stated; artifact usable by a colleague without further explanation.

## Exercise 4 — Integrated capstone
**Scenario:** one realistic problem spanning the whole section.
**Objective:** take the problem through every unit's framework in sequence.
**Constraints:** all unit artifacts required and internally consistent; evidence discipline maintained throughout.
**Deliverable:** a coherent campaign/portfolio document assembled as real professional documentation.
**Evaluation criteria:** consistency across artifacts; each capability demonstrably exercised; trade-offs stated at each decision point.

---

# Part B: Professional Gate Checklists

Use verbatim as module appendices and as workshop job aids. Every item must
force a decision or produce an artifact — items that are merely
inspirational get cut.

## B1. Engineer the CPU GPU boundary checklist
- [ ] Engineer the CPU-GPU boundary through typed-array layouts (interleaved versus planar, quantization with precision budgets), upload mechanics (partial updates, PBO async staging, static-once discipline), and web-worker architectures with zero-copy transferables and OffscreenCanvas rendering?
- [ ] Decisions and their justification are documented for review by a colleague

## B2. Integrate Arrow columnar pipelines checklist
- [ ] Integrate Arrow columnar pipelines and binary transport with progressive loading and caching?
- [ ] Decisions and their justification are documented for review by a colleague

## B3. Manage memory budgets across checklist
- [ ] Manage memory budgets across JS-heap and GPU resources with leak prevention and eviction strategies, orchestrate frame-loop data flow (uniform-only view updates, ring-buffer streaming, backpressure policies), and profile bandwidth-bound versus compute-bound bottlenecks driving optimization targeting?
- [ ] Decisions and their justification are documented for review by a colleague

## B4. Delivery quality checklist
- [ ] Every claim sourced, graded, or flagged as contested?
- [ ] All terminology explained on first use?
- [ ] Trade-offs stated wherever recommendations are context-dependent?
- [ ] Material reads as practitioner-written, not generic?
